# -*- coding: utf-8 -*-
"""
interview-cheat-code.py — 面试作弊工具 · 笔试版（手撕代码 / 算法题详解）
会话制：和 interview-cheat-quiz.py（测评版）同一源码分支，本文件 Alt+P 识图走“详解”模板
...
手撕代码场景走 Alt+P 截屏识图（视觉模型，需 .env 的 ARK_API_KEY；识别带多轮记忆，见热键表）。

热键（自动模式默认）:
  F1        手动录问题兜底（点按开始/结束，走转写提问）
  Alt+P     截屏识图（手撕代码/笔试：题目截图直接出答案。2026-09-06 起改组合键——
            裸 P/F3 会被正常写码敲到/是浏览器查找键，误触风暴；Alt+ 组合零打字冲突。
            识图带多轮记忆：同题续截、报错修复会自动带上此前截图与上次解答）
  Alt+1     答案就位后自动打字打进光标处（再按停止；写码敲 1 不会误触发）
  Alt+2     剪贴板粘贴整段答案（打字链路失效时兜底）
  Alt+3     清空识图多轮记忆（换新题时按；不按也行，模型会按图判断新旧）
  ...
  ESC ×2 / Ctrl+Q  退出（0.8s 内连按两次 ESC，或 Ctrl+Q；单按 ESC 无动作——
                   写码时 IDE 常按 ESC 关补全，单按退出会把工具整没）
...
# 本程序退出全走 os._exit(0)（ESC双按/Ctrl+Q/关闭按钮），finally/atexit 都不执行——
...
def main():
    ...
    def hotkey_loop():
        ...
        while True:
            ...
            hk["esc"] = esc
            # ESC 单独按：退出进程。⚠️ 笔试写码时 IDE/编辑器单按 ESC 极常见（关补全、
            # 取消弹窗）——单按直接整进程退出是事故（2026-09-06 实测被连杀两次）。
            # 改成 0.8s 内连按两次才退；Ctrl+Esc 仍是暂停（上面分支），Ctrl+Q 仍一键退
    ...
    if not args.manual:
        ...
        print("✅ 就绪（自动模式）。双轨全程录音：面试官说话自动断句攒问题，"
              ...
              "F1 手动录问题兜底，Alt+P 截屏识图（同题续截自动带上下文，换题先按 Alt+3 清），"
              "F4 隐藏，F8 提词器，"
              "识图后 Alt+1 自动打进答题框 / Alt+2 整段粘贴，"
              "F9 按住强制收录你的话，F10 附注你的回答开关，↑↓ 翻历史，"
              "ESC×2/Ctrl+Q 退出（0.8s内双按），Ctrl+Esc 暂停", flush=True)
        set_status("🕶️ 防捕获开 · 📱 推送开 · 自动模式")
    else:
        print("✅ 就绪（手动模式）。F1 开始录音 → 面试官提问 → F2 结束 → 转写提问上屏 → DeepSeek 作答。"
              "🕶️ 防捕获常驻开，📱 手机推送常驻开（F7/F6 可关）。"
              "Alt+P 截屏识图（同题续截自动带上下文，换题先按 Alt+3 清），F4 隐藏窗口，F8 提词器，"
              "识图后 Alt+1 自动打进答题框 / Alt+2 整段粘贴，"
              "F10 全听，↑↓ 翻历史，"
              "ESC×2/Ctrl+Q 退出（0.8s内双按），Ctrl+Esc 暂停", flush=True)
