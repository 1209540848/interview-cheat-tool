...
2026-09 模块化重构：两个功能等价的旧单体（`interview-cheat-quiz.py` / `interview-cheat-code.py`，已从工作区移除，历史在 git 中）收敛为 **功能域包 + 两个薄入口**：
...
├─ interview-cheat-api.py      # API 祖本：一字不动（遗留，独立演进）
...
- 每次搬移/收敛改动过包模块后跑门禁：`python -m py_compile run_quiz.py run_code.py interview_cheat/*.py` + `python tools/parity.py`（改动需 ⊆ allow 清单）。
